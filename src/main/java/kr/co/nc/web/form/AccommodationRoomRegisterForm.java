...
public class AccommodationRoomRegisterForm {
	...
	private List<String> detailImageNames = new ArrayList<>(); // 상세이미지 리스트
	private List<MultipartFile> detailImageFiles = new ArrayList<>(); // 상세이미지 리스트
	private List<String> stringRoomFacilities = new ArrayList<>(); // 이 객실이 가지는 모든 객실시설 정보
	private List<RoomFacility> roomFacilities = new ArrayList<>(); // 이 객실이 가지는 모든 객실시설 정보
}
